...
from vault import storage_types, memory_storage
from storage import types


class TestMemoryStorage(TestCase):
    def test_entity_storage(self):
        record_storage: types.IEntityStorage[storage_types.StorageRecord] = memory_storage.InMemoryEntityStorage ()
        record_key_storage: types.ILinkStorage[storage_types.StorageRecordKey] = memory_storage.InMemoryLinkStorage()
        ...
        link.record_key = utils.base64_url_encode(crypto.encrypt_aes_v2(record_key, client_key))
        link.can_edit = True
