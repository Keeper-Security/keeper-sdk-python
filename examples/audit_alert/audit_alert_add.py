...
#
# Example showing how to add a new audit alert
...
def execute_audit_alert_add(context: KeeperParams, **kwargs):
    ...
    This function creates a new audit alert
    using the Keeper CLI command infrastructure.
    """
    audit_alert_add_command = AuditAlertAdd()
    
    try:
        audit_alert_add_command.execute(context=context, **kwargs)
...
Example:
  python audit_alert_add.py
    ...
    try:
        execute_audit_alert_add(context, **kwargs)
        print('Audit alert add completed successfully')
        sys.exit(0)
    except Exception as e:
        print(f'Error: {str(e)}')
        sys.exit(1)
