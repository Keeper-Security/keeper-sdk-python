...
def main():
    ...
    try:
        print(f'Account-unlocking {len(device_identifiers)} device(s)...')
        for name in device_management.account_unlock_user_devices(
            keeper_auth_context, device_identifiers
        ):
            print(f"Device '{name}' successfully account unlocked")
        print('\nUpdated device list:')
        print_devices_table(device_management.list_user_devices(keeper_auth_context))
    except Exception as e:
        print(f'Error account-unlocking devices: {e}')
    finally:
