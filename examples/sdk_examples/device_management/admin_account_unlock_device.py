...
def main():
    ...
    try:
        print(f'Account-unlocking {len(device_identifiers)} device(s) for user {enterprise_user_id}...')
        for name in device_management.account_unlock_admin_user_devices(
            keeper_auth_context, enterprise_user_id, device_identifiers
        ):
            print(
                f"Device action successfully completed: '{name}' account unlocked "
        ...
        print(f'Error account-unlocking admin devices: {e}')
    finally:
