...
def main():
    ...
    try:
        print(f'Removing {len(device_identifiers)} device(s) for user {enterprise_user_id}...')
        for name in device_management.remove_admin_user_devices(
            keeper_auth_context, enterprise_user_id, device_identifiers
        ):
            print(
                f"Device action successfully completed: '{name}' removed "
        ...
        print(f'Error removing admin devices: {e}')
    finally:
