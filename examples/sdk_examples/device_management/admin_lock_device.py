...
def main():
    ...
    try:
        print(f'Locking {len(device_identifiers)} device(s) for user {enterprise_user_id}...')
        for name in device_management.lock_admin_user_devices(
            keeper_auth_context, enterprise_user_id, device_identifiers
        ):
            print(
                f"Device action successfully completed: '{name}' locked "
        ...
        print(f'Error locking admin devices: {e}')
    finally:
