...
def main():
    ...
    try:
        print(f'Locking {len(device_identifiers)} device(s)...')
        for name in device_management.lock_user_devices(
            keeper_auth_context, device_identifiers
        ):
            print(f"Device '{name}' successfully locked")
        print('\nUpdated device list:')
        print_devices_table(device_management.list_user_devices(keeper_auth_context))
    except Exception as e:
        print(f'Error locking devices: {e}')
    finally:
