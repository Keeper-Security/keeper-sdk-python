...
def main():
    ...
    device_identifiers = ['<device_id_or_name_1>', '<device_id_or_name_2>']

    try:
        print(f'Unlinking {len(device_identifiers)} device(s)...')
        for name in device_management.unlink_user_devices(
            keeper_auth_context, device_identifiers
        ):
            print(f"Device '{name}' successfully unlinked")
        print('\nUpdated device list:')
        print_devices_table(device_management.list_user_devices(keeper_auth_context))
    except Exception as e:
        print(f'Error unlinking devices: {e}')
    finally:
