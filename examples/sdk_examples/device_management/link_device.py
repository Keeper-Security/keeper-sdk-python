...
def main():
    ...
    try:
        print(f'Linking {len(device_identifiers)} device(s)...')
        for name in device_management.link_user_devices(
            keeper_auth_context, device_identifiers
        ):
            print(f"Device '{name}' successfully linked")
        print('\nUpdated device list:')
        print_devices_table(device_management.list_user_devices(keeper_auth_context))
    except Exception as e:
        print(f'Error linking devices: {e}')
    finally:
