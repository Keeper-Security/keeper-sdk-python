...
def print_admin_devices_table(devices):
    if not devices:
        print('\nNo devices found.')
        return
    print(f'\nAdmin Device List ({len(devices)} devices found)')
    print('=' * 120)
    print(
        f"{'ID':<4} {'Enterprise User ID':<20} {'Device Name':<22} "
        f"{'UI Category':<18} {'Device Status':<16} {'Login Status':<14} {'Last Accessed':<20}"
    )
    print('-' * 120)
    for d in devices:
        last = d.last_accessed.strftime('%Y-%m-%d %H:%M:%S') if d.last_accessed else 'N/A'
        print(
            f"{d.list_index:<4} {d.enterprise_user_id:<20} {d.name[:21]:<22} "
            f"{d.ui_category[:17]:<18} {d.device_status[:15]:<16} "
            f"{d.login_status[:13]:<14} {last:<20}"
        )
    print('-' * 120)
...
def main():
    ...
    try:
        print(f'Account-locking {len(device_identifiers)} device(s) for user {enterprise_user_id}...')
        for name in device_management.account_lock_admin_user_devices(
            keeper_auth_context, enterprise_user_id, device_identifiers
        ):
            print(
                f"Device action successfully completed: '{name}' account locked "
                f'for user {enterprise_user_id}'
            )
        print(f'\nUpdated device list for user {enterprise_user_id}:')
        print_admin_devices_table(
            device_management.list_admin_devices(keeper_auth_context, [enterprise_user_id])
        )
    except Exception as e:
        print(f'Error account-locking admin devices: {e}')
    finally:
