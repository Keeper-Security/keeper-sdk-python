...
def main():
    ...
    try:
        print(f'Logging out {len(device_identifiers)} device(s) for user {enterprise_user_id}...')
        for name in device_management.logout_admin_user_devices(
            keeper_auth_context, enterprise_user_id, device_identifiers
        ):
            print(
                f"Device action successfully completed: '{name}' logged out "
        ...
        print(f'Error logging out admin devices: {e}')
    finally:
