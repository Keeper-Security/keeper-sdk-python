...
def main():
    ...
    try:
        print(f'Unlocking {len(device_identifiers)} device(s) for user {enterprise_user_id}...')
        for name in device_management.unlock_admin_user_devices(
            keeper_auth_context, enterprise_user_id, device_identifiers
        ):
            print(
                f"Device action successfully completed: '{name}' unlocked "
        ...
        print(f'Error unlocking admin devices: {e}')
    finally:
