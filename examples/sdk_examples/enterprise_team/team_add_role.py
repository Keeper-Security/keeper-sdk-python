...
def main():
    ...
    try:
        ...
        if not team:
            ...
            return
        role = enterprise_user_management.resolve_role(enterprise_data, role_name_or_id)

        if any(enterprise_data.managed_nodes.get_links_by_subject(role.role_id)):
            print('Teams cannot be assigned to roles with administrative permissions.')
            return

        existing_roles = {
            x.role_id
            for x in enterprise_data.role_teams.get_links_by_object(team.team_uid)
        }
        if role.role_id in existing_roles:
            print(f"Role '{role.name}' is already assigned to team '{team.name}'")
            return

        batch = batch_management.BatchManagement(loader=loader)
        batch.modify_role_teams(to_add=[
            enterprise_management.RoleTeamEdit(role_id=role.role_id, team_uid=team.team_uid)
        ])
        batch.apply()
        print(f"Added role '{role.name}' to team '{team.name}'")
    except Exception as e:
