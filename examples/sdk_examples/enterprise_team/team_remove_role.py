...
def main():
    ...
    try:
        ...
        if role.role_id not in existing_roles:
            print(f"Role '{role.name}' is not assigned to team '{team.name}'")
            return

        batch = batch_management.BatchManagement(loader=loader)
        batch.modify_role_teams(to_remove=[
            enterprise_management.RoleTeamEdit(role_id=role.role_id, team_uid=team.team_uid)
        ])
        batch.apply()
        print(f"Removed role '{role.name}' from team '{team.name}'")
    except Exception as e:
        print(f'Error removing role from team: {e}')
    finally:
