...
from keepersdk.enterprise import enterprise_loader, enterprise_team_management, sqlite_enterprise_storage
...
def _load_enterprise(keeper_auth_context: keeper_auth.KeeperAuth) -> enterprise_loader.EnterpriseLoader:
    ...
    return enterprise_loader.EnterpriseLoader(keeper_auth_context, enterprise_storage)


def _load_vault(keeper_auth_context: keeper_auth.KeeperAuth) -> vault_online.VaultOnline:
    conn = sqlite3.Connection('file::memory:', uri=True)
    storage = sqlite_storage.SqliteVaultStorage(
        lambda: conn, keeper_auth_context.auth_context.account_uid
    )
    vault = vault_online.VaultOnline(keeper_auth_context, storage)
    vault.sync_down()
    return vault


def print_team_info(team_info: enterprise_team_management.EnterpriseTeamInfo) -> None:
    print(f"\nTeam UID: {team_info.team_uid}")
    print(f"Team Name: {team_info.team_name}")
    if team_info.node_name:
        print(f"Node: {team_info.node_name} [{team_info.node_id}]")
    print(f"Access Level: {team_info.access_level}")
    print(f"Restrict Edit: {team_info.restrict_edit}")
    print(f"Restrict Share: {team_info.restrict_share}")
    print(f"Restrict View: {team_info.restrict_view}")

    if team_info.team_roles:
        print(f"Role(s): {', '.join(x.role_name for x in team_info.team_roles)}")

    if team_info.team_users:
        print(f"User(s): {', '.join(x.username for x in team_info.team_users)}")

    if team_info.queued_team_users:
        print(f"Queued User(s): {', '.join(x.username for x in team_info.queued_team_users)}")

    if team_info.members:
        print(f"\n{'Enterprise User ID':<20} {'Email':<40} {'Share Admin':<12}")
        print('-' * 74)
        for member in team_info.members:
            print(
                f"{member.enterprise_user_id:<20} "
                f"{member.email[:39]:<40} "
                f"{'Yes' if member.is_share_admin else 'No':<12}"
            )
...
def main():
    ...
    is_admin = bool(keeper_auth_context.auth_context.is_enterprise_admin)
    enterprise = None
    vault = None

    try:
        if is_admin:
            enterprise = _load_enterprise(keeper_auth_context)
        vault = _load_vault(keeper_auth_context)
        ...
        print(f'Error getting team: {exc}')
