...
def print_teams_table(summaries) -> None:
    if not summaries:
        print('\nNo teams found.')
        return

    print(f'\nEnterprise Teams ({len(summaries)} found)')
    print('=' * 120)
    print(
        f"{'Team Name':<30} {'Team UID':<28} {'Node':<25} "
        f"{'Users':<8} {'Roles':<8} {'Restricts':<12}"
    )
    print('-' * 120)
    for team in summaries:
        restricts = (
            f"{'R' if team.restrict_view else '-'}"
            f"{'W' if team.restrict_edit else '-'}"
            f"{'S' if team.restrict_share else '-'}"
        )
        print(
            f"{team.team_name[:29]:<30} {team.team_uid[:27]:<28} "
            f"{team.node_name[:24]:<25} {team.user_count:<8} {team.role_count:<8} {restricts:<12}"
        )
    print('-' * 120)
...
def main():
    ...
    pattern = ''  # Optional filter, e.g. 'Testing Team'

    enterprise = None
    try:
        enterprise = _load_enterprise(keeper_auth_context)
        summaries = enterprise_team_management.list_teams(
            enterprise.enterprise_data,
            pattern=pattern or None,
        )
        print_teams_table(summaries)
    except Exception as exc:
        print(f'Error listing teams: {exc}')
    finally:
