...
def login():
    """
    Handle the login process including authentication and multi-factor authentication steps.
    ...
    while not login_auth_context.login_step.is_final():
        if isinstance(login_auth_context.login_step, login_auth.LoginStepDeviceApproval):
            ...
            raise NotImplementedError(f"Unsupported login step type: {type(login_auth_context.login_step).name}")
    ...
    if isinstance(login_auth_context.login_step, login_auth.LoginStepConnected):
        return login_auth_context.login_step.take_keeper_auth(), keeper_endpoint
