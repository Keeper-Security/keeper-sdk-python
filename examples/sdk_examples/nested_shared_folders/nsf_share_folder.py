...
def nsf_share_folder(vault: vault_online.VaultOnline) -> None:
    """Grant or revoke NSF folder access (nsf-share-folder)."""
...
def nsf_share_folder_run(keeper_auth_context: keeper_auth.KeeperAuth) -> None:
    vault = open_vault(keeper_auth_context)
    try:
        nsf_share_folder(vault)
    except Exception as e:
        print(f"Error: {e}")
    finally:
...
def main() -> None:
    ...
    if keeper_auth_context:
        nsf_share_folder_run(keeper_auth_context)
    else:
        print("Login failed.")
