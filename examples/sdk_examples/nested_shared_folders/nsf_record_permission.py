...
from keepersdk.vault import nsf_sharing, sqlite_storage, vault_online
...
def open_vault(keeper_auth_context: keeper_auth.KeeperAuth) -> vault_online.VaultOnline:
...
def close_vault(vault: vault_online.VaultOnline, keeper_auth_context: keeper_auth.KeeperAuth) -> None:
    vault.close()
    keeper_auth_context.close()

def nsf_record_permission(vault: vault_online.VaultOnline, current_user: str) -> None:
    """Bulk-update NSF record permissions within a folder (nsf-record-permission)."""
...
def nsf_record_permission_run(keeper_auth_context: keeper_auth.KeeperAuth) -> None:
    vault = open_vault(keeper_auth_context)
    try:
        nsf_record_permission(
            vault, keeper_auth_context.auth_context.username
        )
    except Exception as e:
        print(f"Error: {e}")
    finally:
...
def main() -> None:
    ...
    if keeper_auth_context:
        nsf_record_permission_run(keeper_auth_context)
    else:
        print("Login failed.")
