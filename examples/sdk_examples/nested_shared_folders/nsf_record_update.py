...
def nsf_record_update(vault: vault_online.VaultOnline) -> None:
    """Update an NSF record (nsf-record-update)."""
    RECORD_UID_OR_TITLE = "My NSF Login"
    NEW_TITLE = "My NSF Login (updated)"
    FIELDS = {"password": "new-secret-value"}
    NOTES = "Updated via SDK example"
...
def nsf_record_update_run(keeper_auth_context: keeper_auth.KeeperAuth) -> None:
    vault = open_vault(keeper_auth_context)
    try:
        nsf_record_update(vault)
    except Exception as e:
        print(f"Error: {e}")
    finally:
...
def main() -> None:
    ...
    if keeper_auth_context:
        nsf_record_update_run(keeper_auth_context)
    else:
        print("Login failed.")
