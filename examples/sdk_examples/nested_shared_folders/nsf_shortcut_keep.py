...
from keepersdk.vault import nsf_folder_records, nsf_management, sqlite_storage, vault_online
...
def nsf_shortcut_keep(vault: vault_online.VaultOnline) -> None:
    """Keep an NSF record in one folder only (nsf-shortcut keep)."""
...
def nsf_shortcut_keep_run(keeper_auth_context: keeper_auth.KeeperAuth) -> None:
    vault = open_vault(keeper_auth_context)
    try:
        nsf_shortcut_keep(vault)
    except Exception as e:
        print(f"Error: {e}")
    finally:
...
def main() -> None:
    ...
    if keeper_auth_context:
        nsf_shortcut_keep_run(keeper_auth_context)
    else:
        print("Login failed.")
