...
from keepersdk.vault import nsf_folder_records, sqlite_storage, vault_online
...
def nsf_shortcut_list(vault: vault_online.VaultOnline) -> None:
    """List NSF records linked to multiple folders (nsf-shortcut list)."""
...
def nsf_shortcut_list_run(keeper_auth_context: keeper_auth.KeeperAuth) -> None:
    vault = open_vault(keeper_auth_context)
    try:
        nsf_shortcut_list(vault)
    except Exception as e:
        print(f"Error: {e}")
    finally:
...
def main() -> None:
    ...
    if keeper_auth_context:
        nsf_shortcut_list_run(keeper_auth_context)
    else:
        print("Login failed.")
