...
def nsf_record_details(vault: vault_online.VaultOnline) -> None:
    """Get NSF record metadata via v3 API (nsf-record-details)."""
...
def nsf_record_details_run(keeper_auth_context: keeper_auth.KeeperAuth) -> None:
    vault = open_vault(keeper_auth_context)
    try:
        nsf_record_details(vault)
    except Exception as e:
        print(f"Error: {e}")
    finally:
...
def main() -> None:
    ...
    if keeper_auth_context:
        nsf_record_details_run(keeper_auth_context)
    else:
        print("Login failed.")
