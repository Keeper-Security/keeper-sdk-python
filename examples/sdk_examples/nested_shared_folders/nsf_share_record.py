...
def nsf_share_record(vault: vault_online.VaultOnline) -> None:
    """Change NSF record sharing permissions (nsf-share-record)."""
...
def nsf_share_record_run(keeper_auth_context: keeper_auth.KeeperAuth) -> None:
    vault = open_vault(keeper_auth_context)
    try:
        nsf_share_record(vault)
    except Exception as e:
        print(f"Error: {e}")
    finally:
...
def main() -> None:
    ...
    if keeper_auth_context:
        nsf_share_record_run(keeper_auth_context)
    else:
        print("Login failed.")
