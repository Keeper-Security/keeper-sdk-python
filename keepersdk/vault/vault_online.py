...
class VaultOnline(vault_data.VaultData):
    def __init__(self, keeper_auth, storage):
        # type: (auth.KeeperAuth, vault_storage.IVaultStorage) -> None
        super(VaultOnline, self).__init__(keeper_auth.auth_context.client_key, storage)
        self._keeper_auth = keeper_auth
        self._auto_sync = False
        self._sync_down_lock = threading.Lock()
        self._executor = concurrent.futures.ThreadPoolExecutor(max_workers=1)
        self._sync_record_types = True
        self._pending_audit_data = None  # type: Optional[Dict[str, record_pb2.RecordAddAuditData]]
        self._pending_audit_events = []  # type: List[Dict[str, Any]]
        self.sync_requested = False
        self.auto_sync = True    # call setter

    def close(self):
        if self.keeper_auth:
            self._keeper_auth.close()
        super(VaultOnline, self).close()

    @property
    def keeper_auth(self):     # type: () -> auth.KeeperAuth
        return self._keeper_auth

    @property
    def auto_sync(self):    # type: () -> bool
        return self._auto_sync

    @auto_sync.setter
    def auto_sync(self, value):    # type: (bool) -> None
    ...
    def on_notification_received(self, event):   # type: (Dict[str, Any]) -> Optional[bool]
    ...
    @staticmethod
    def process_add_audit_data_response(fut):    # type: (concurrent.futures.Future) -> None
    ...
    @staticmethod
    def process_audit_event_response(fut):    # type: (concurrent.futures.Future) -> None
        if fut.done() and not fut.cancelled():
            e = fut.exception(0)
            if e:
    ...
    def schedule_audit_data(self, record, revision):  # type: (Union[PasswordRecord, TypedRecord], int) -> None
