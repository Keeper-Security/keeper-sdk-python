...
class _DataType(abc.ABC):
    @abc.abstractmethod
    def get_proto_entity_key(self, entity):   # type: (Any) -> str
        pass


class _MergeDataType(_DataType, abc.ABC):
    @abc.abstractmethod
    def merge(self, existing, new, is_delete):   # type: (Optional[Any], Any, bool) -> Tuple[Any, bool]
        pass


class _SimpleDataType(_DataType):
    def __init__(self, keys):   # type: (Sequence[str]) -> None
        self.keys = keys
...
class _EnterpriseEntityStorage(sqlite_dao.SqliteStorage):
    def __init__(self, get_connection, schema, owner=None):
        # type: (Callable[[], sqlite3.Connection], sqlite_dao.TableSchema, Optional[sqlite_dao.KeyTypes]) -> None
    ...
    def delete_entities(self, keys):   # type: (Iterable[_EntityKey]) -> None
...
class SqliteEnterpriseStorage(EnterpriseStorage):
    def __init__(self, get_connection, enterprise_id):   # type: (Callable[[], sqlite3.Connection], int) -> None
        self.get_connection = get_connection
        self.enterprise_id = enterprise_id
        self.owner_column = 'enterprise_id'
        self._put_cache = {}          # type: Dict[_EntityKey, EnterpriseEntityData]
        self._delete_cache = set()    # type: Set[_EntityKey]
