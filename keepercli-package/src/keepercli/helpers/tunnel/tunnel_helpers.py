...
from ...compat.folder_mixin import FolderMixin
...
from ..router_utils import router_get_relay_access_creds, get_dag_leafs, \
    get_router_ws_url, get_router_url, router_send_action_to_gateway
from ...compat.display import bcolors
from ...commands.base import CommandError
from ..folder_utils import try_resolve_path
from keepersdk import crypto, utils
from keepersdk.authentication import endpoint as rest_api
from ...compat import api
from ...compat.constants import get_relay_host
...
def _find_pam_config_records(params):
    """Scan vault for PAM configuration records (version 6)."""
    if params.vault is None:
        return
    for info in params.vault.vault_data.find_records(record_type='pamConfiguration', record_version=6):
        kr = params.vault.vault_data.load_record(info.record_uid)
        if kr is not None:
            yield kr
...
def get_config_uid_from_local_vault(params, record_uid):
    ...
    try:
        ...
        return None
    try:
        for kr in _find_pam_config_records(params):
...
def get_config_uid_via_pam_link(params, record_uid):
    ...
    """
    try:
        from keepersdk.proto import GraphSync_pb2 as gs_pb2
        from ..router_utils import _post_request_to_router_for_params as _post_request_to_router
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if router_response is None:
                    raise Exception('This Gateway currently is not online.')
