...
def try_resolve_path(context: KeeperParams, path: str) -> Tuple[vault_types.Folder, str]:
    ...
    folder: Optional[vault_types.Folder] = context.vault.get_folder(path)
    ...
    if path.startswith('/') and not path.startswith('//'):
        ...
        folder = context.vault.get_folder(context.current_folder)
    if folder is None:
        folder = context.vault.root_folder
    ...
    while len(components) > 0:
        ...
        if component == '..':
            ...
            if parent_uid:
                f = context.vault.get_folder(parent_uid)
                if f:
                    folder = f
            else:
                folder = context.vault.root_folder
            ...
            if component in folder.subfolders:
                f = context.vault.get_folder(component)
                if f:
                    folder = f
            else:
                folders = [f for f in (context.vault.get_folder(x) for x in folder.subfolders) if f]
