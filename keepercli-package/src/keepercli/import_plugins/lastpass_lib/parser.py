...
def parse_ACCT(chunk, encryption_key, shared_folder) -> Account:
    """
    ...
    account_id = read_item(io).decode('utf-8')
    ...
    account = Account(account_id, name, username, password, url, group, notes, shared_folder, attach_key, totp_secret, totp_url)
...
def parse_SHAR(chunk: Any, encryption_key: bytes, rsa_key: bytes) -> Any:
...
def parse_ATTA(chunk):
    io = BytesIO(chunk.payload)
    attachment_id = read_item(io).decode('utf-8', 'ignore')
    ...
    return LastpassAttachment(attachment_id, parent_id, mimetype, storagekey, lastpass_size, filename_encrypted)
...
def read_item(stream: BytesIO) -> bytes:
