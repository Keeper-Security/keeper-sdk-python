...
headers = {'user-agent': 'lastpass_lib-python/{}'.format(__version__)}
...
def logout(session):   # type: (Session) -> None
    response = requests.get('https://lastpass.com/logout.php?mobile=1', cookies={'PHPSESSID': session.id},
                              proxies=endpoint.get_proxies(), verify=endpoint.get_certificate_check())
...
def fetch(session):
    ...
    response = requests.get(url, cookies={'PHPSESSID': session.id},
                              proxies=endpoint.get_proxies(), verify=endpoint.get_certificate_check())
...
def stream_attachment(session, attach_info):
    ...
    response = requests.post(url, data=data, cookies={'PHPSESSID': session.id}, stream=True,
                               proxies=endpoint.get_proxies(), verify=endpoint.get_certificate_check())
...
def fetch_shared_folder_members(session, shareid):
    url = f'{https_host}/getSharedFolderMembers.php?{query_string}&shareid={shareid}'
    response = requests.get(url, cookies={'PHPSESSID': session.id}, proxies=endpoint.get_proxies(),
                              verify=endpoint.get_certificate_check())
...
def request_iteration_count(username):
    response = requests.get('https://lastpass.com/iterations.php', params={'email': username}, headers=headers,
                              proxies=endpoint.get_proxies(), verify=endpoint.get_certificate_check())
    if response.status_code != requests.codes.ok:
        raise NetworkError()
