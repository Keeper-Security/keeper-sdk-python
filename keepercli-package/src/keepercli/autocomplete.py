...
def standard_completer(context: KeeperParams) -> Callable[[str, str], Iterable[str]]:
    def completer(meta: str, to_complete: str) -> Iterable[str]:

        if to_complete and to_complete[0] in {'"', "'"}:
            prefix = to_complete[1:]
            quote = to_complete[0]
        else:
            prefix = to_complete
            quote = ''

        meta_completer: Optional[Callable[[KeeperParams, str], Iterable[str]]] = None
        if meta == 'FOLDER':
...
def split_text(text: str) -> List[Tuple[int, int]]:
    ...
    for i in range(len(text)):
        ...
        if is_escape:
            ...
            if word_pos >= 0 and last_quote is None and not is_escape:
                words.append((word_pos, i - 1))
                word_pos = -1
        elif ch in '\'"':
            if word_pos < 0:
                word_pos = i
            ...
            if word_pos < 0:
                word_pos = i
    if word_pos >= 0:
        words.append((word_pos, len(text) - 1))
