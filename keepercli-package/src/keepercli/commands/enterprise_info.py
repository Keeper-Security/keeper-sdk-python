...
class EnterpriseInfoTreeCommand(base.ArgparseCommand, enterprise_command.EnterpriseMixin):
    ...
    def execute(self, context: KeeperParams, **kwargs):
        ...
        subnodes = self.get_subnodes(context)
        root_nodes: Dict[int, bool] = self.get_managed_nodes_for_user(context, context.auth.auth_context.username)
        managed_nodes = self.expand_managed_nodes(root_nodes, subnodes)
        if kwargs.get('node'):
            accessible_nodes: Set[int] = set()
            for node_id, nodes in managed_nodes.items():
                accessible_nodes.update(nodes)
            ...
            managed_nodes = self.filter_managed_nodes(context, managed_nodes, root_node[0])

        nodes = context.enterprise_data.nodes
        verbose = kwargs.get('verbose') is True
        users: Dict[int, List[enterprise_types.User]] = {}
        for user in context.enterprise_data.users.get_all_entities():
            if user.node_id not in managed_nodes:
                continue
        ...
        for role in context.enterprise_data.roles.get_all_entities():
            if role.node_id not in managed_nodes:
                continue
        ...
        for team in context.enterprise_data.teams.get_all_entities():
            if team.node_id not in managed_nodes:
                continue
        ...
        for queued_team in context.enterprise_data.queued_teams.get_all_entities():
            if queued_team.node_id not in managed_nodes:
                continue
        ...
        def tree_node(node: enterprise_types.Node) -> Tuple[str, Dict[str, dict]]:
            ...
            children = [x for x in (nodes.get_entity(y) for y in subnodes.get(node_id, set())) if x is not None]
            ...
            node_queued_teams = queued_teams.get(node_id)
        ...
        for node_id in managed_nodes:
            node = context.enterprise_data.nodes.get_entity(node_id)
            if not node:
                continue
