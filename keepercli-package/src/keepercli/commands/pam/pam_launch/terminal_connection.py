...
from .... import api
from keepersdk.helpers.keeper_dag.dag_types import EdgeType
from keepersdk.proto.APIRequest_pb2 import GetKsmPublicKeysRequest, GetKsmPublicKeysResponse
...
from ..pam_dto import GatewayAction, GatewayActionWebRTCSession
from ....helpers.router_utils import (
...
def _get_launch_credential_uid(
    ...
    try:
        if tdag is None:
            ...
            tdag = TunnelDAG(params.vault, encrypted_session_token, encrypted_transmission_key, record_uid,
        ...
        try:
            ...
            if trickle_ice:
                ...
                if router_response is None:
                    signal_handler.cleanup()
                    unregister_tunnel_session(commander_tube_id)
                    # Fail fast (Commander waits ~24s for WebRTC). Keep the same
                    # user-facing offline wording; avoid nested CommandError wrapping.
                    return {
                        "success": False,
                        "error": "This Gateway currently is not online.",
                    }
...
def launch_terminal_connection(params: KeeperParams, 
    ...
    try:
        ...
        if not tunnel_result.get('success'):
            ...
            # Offline gateway: surface Commander's wording without the WebRTC wrapper.
            if 'not online' in str(error_msg).lower():
                raise CommandError('pam launch', error_msg)
