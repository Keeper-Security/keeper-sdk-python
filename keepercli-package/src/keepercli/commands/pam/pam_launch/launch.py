...
from ....helpers.tunnel.tunnel_helpers import (
...
from ....helpers.router_utils import router_get_connected_gateways
from ....helpers.ssh_agent import try_extract_private_key
from .... import api
from ....compat import vault
from ....helpers.folder_utils import try_resolve_path
...
class PAMLaunchCommand(ArgparseCommand):
    ...
    def __init__(self):
        ...
        PAMLaunchCommand.add_arguments_to_parser(parser)
    ...
    def find_record(self, params: KeeperParams, record_token: str) -> Optional[str]:
        ...
        if uid_pattern.match(record_token):
            if record_token in params.record_cache:
                ...
                return record_token
            if params.vault is not None:
                if params.vault.vault_data.get_record(record_token):
                    logging.debug(f"Found record by vault UID: {record_token}")
                    return record_token
                if params.vault.nsf_data and params.vault.nsf_data.get_record(record_token):
                    logging.debug(f"Found record by NSF UID: {record_token}")
                    return record_token
    ...
    def find_gateway(
        ...
        all_gateways = get_all_gateways(params.vault)
        ...
        if not gateway_proto:
            ...
            if config_uid:
                try:
                    from keepersdk.helpers.config_utils import configuration_controller_get
                    controller = configuration_controller_get(
                        params.vault, url_safe_str_to_bytes(config_uid),
                    )
    ...
    def _gateway_uid_from_config(params: KeeperParams, pam_config_uid: str) -> str:
        ...
        if not gateway_uid:
            try:
                ...
                controller = configuration_controller_get(params.vault, config_uid_bytes)
    ...
    def execute(self, context: KeeperParams, **kwargs):
        from ....compat.commander_params import ensure_commander_compat
        ensure_commander_compat()
        params = context
        """
        ...
        try:
            ...
            if _cache_entry is not None:
                ...
                def _refresh_fetch(_params=params, _record_uid=record_uid, _self=self):
                    try:
                        ...
                        _tdag = TunnelDAG(
                            _params.vault, _enc_s, _enc_t, _record_uid, transmission_key=_tk,
                        )
            ...
            # Runs on cache hit too so the offline warning is not skipped.
            try:
                connected_gateways = router_get_connected_gateways(params.vault)
        ...
        try:
            ...
            if (workflow_started_by_launch and not lease_expired
                ...
                try:
                    ...
                    from ....helpers.router_utils import _post_request_to_router_for_params as _post_request_to_router
