...
def print_exempt_message(fmt='table'):
    ...
    from ...compat.display import bcolors as _bc
    if fmt == 'json':
...
def is_workflow_exempt(params, record_uid, config=None):
    ...
    if config is None:
        from ...helpers.router_utils import _post_request_to_router
        try:
...
def is_pam_config_action_allowed_for_record(params: KeeperParams, record_uid: str,
    ...
    try:
        config_uid = None
        try:
            from ..pam.pam_launch import launch_cache
        ...
        if not config_uid:
            from ...compat import vault as _vault
            from ...helpers.tunnel.tunnel_helpers import get_config_uid_from_record
            try:
        ...
        from ...helpers import pam_config_utils as _pam_config_utils
        allowed = pam_config_utils.pam_config_allowed_settings_json(
            params, config_uid,
        )
