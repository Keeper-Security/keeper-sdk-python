...
from ...commands.base import CommandError, argparse, GroupCommand, ArgparseCommand
...
class BiometricArgparseCommand(ArgparseCommand):
    """Base class for biometric commands that need argument parsing"""
    ...
    def _check_platform_support(self, force: bool = False):
        ...
        if not FIDO2_AVAILABLE:
            raise CommandError(ERROR_MESSAGES['no_fido2'])

        supported, message = self.detector.detect_platform_capabilities()

        if not supported and not force:
            raise CommandError(f'{ERROR_MESSAGES["platform_not_supported"]}: {message}')
    ...
    def _get_available_credentials_or_error(self, vault: vault_online.VaultOnline):
        """Get available credentials with consistent error handling"""
        try:
            credentials = self.client.get_available_credentials(vault)
            if not credentials:
                raise CommandError(ERROR_MESSAGES['no_credentials'])
            return credentials
