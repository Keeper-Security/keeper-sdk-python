...
from keepersdk import errors, crypto, utils
from keepersdk.authentication import configuration, login_auth, endpoint, notifications
...
class TestLogin(TestCase):
    ...
    def mock_execute_rest(keeper_endpoint, rest_endpoint, request=None, response_type=None, session_token=None, payload_version=None):
        ...
        if rest_endpoint == 'authentication/start_login':
            ...
            if TestLogin.StopAtDeviceApproval:
                ...
                pk = crypto.load_ec_private_key(utils.base64_url_decode(dc.private_key))
                lrs.encryptedDataKey = crypto.encrypt_ec(data_vault.UserDataKey, pk.public_key())
        ...
        if rest_endpoint == 'authentication/validate_device_verification_code':
            dvrq:  APIRequest_pb2.ValidateDeviceVerificationCodeRequest = request
        ...
        if rest_endpoint == 'authentication/2fa_validate':
            tfarq:  APIRequest_pb2.TwoFactorValidateRequest = request
            if tfarq.value == data_vault.TwoFactorOneTimeToken:
                ...
                raise errors.KeeperApiError('auth_failed', 'Invalid code')
        if rest_endpoint == 'authentication/validate_auth_hash':
            prq:  APIRequest_pb2.ValidateAuthHashRequest = request
            ...
            if expected_auth_hash == prq.authResponse:
                ...
                raise errors.KeeperApiError('auth_failed', 'invalid password')
    ...
    def get_auth_sync():
        ...
        def execute_rest(rest_endpoint, request=None, response_type=None, session_token=None, payload_version=None):
