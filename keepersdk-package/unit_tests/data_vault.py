...
from keepersdk.authentication import configuration, keeper_auth, endpoint
...
def generate_account_summary(keeper_endpoint: endpoint.KeeperEndpoint) -> AccountSummary_pb2.AccountSummaryElements:
    ...
    for dc in config.devices().list():
        ...
        device_ec_pk = crypto.load_ec_private_key(utils.base64_url_decode(dc.private_key))
        di.devicePublicKey = crypto.unload_ec_public_key(device_ec_pk.public_key())
    return result


def sync_down_response() -> SyncDown_pb2.SyncDownResponse:
    keys: Dict[bytes, bytes] = {}
    records: List[SyncDown_pb2.Record] = []
    meta_data: List[SyncDown_pb2.RecordMetaData] = []
    nsd: List[SyncDown_pb2.NonSharedData] = []
    for_sf: List[SyncDown_pb2.Record] = []
    ...
    shared_folders: List[SyncDown_pb2.SharedFolder] = []
    shared_folder_records: List[SyncDown_pb2.SharedFolderRecord] = []
    record_keys: List[Tuple[SyncDown_pb2.Record, bytes]] = []
    for_team: List[SyncDown_pb2.SharedFolder] = []
    for shared_folder in get_test_shared_folders():
        use_key = len(shared_folders) == 0
        sfd, sfk = generate_shared_folder(shared_folder, use_key)
        if not use_key:
            for_team.append(sfd)
        keys[sfd.sharedFolderUid] = sfk
        shared_folders.append(sfd)
        record_keys.clear()
        ...
        if len(record_keys) > 0:
            shared_folder_records.extend(generate_shared_folder_records((sfd.sharedFolderUid, sfk), record_keys))

    teams: List[SyncDown_pb2.Team] = []
    shared_folder_teams: List[SyncDown_pb2.SharedFolderTeam] = []
...
def generate_record(record: vault_record.KeeperRecord) -> Tuple[SyncDown_pb2.Record, bytes]:
    record_key = utils.generate_aes_key()
    if isinstance(record, vault_record.PasswordRecord):
        data, extra, _ = vault_extensions.extract_password_record(record)
...
def generate_team(team: vault_types.Team,
                  shared_folders: Iterable[Tuple[SyncDown_pb2.SharedFolder, bytes]]
                  ) -> Tuple[SyncDown_pb2.Team, List[SyncDown_pb2.SharedFolderTeam], bytes]:
    ...
    sfts: List[SyncDown_pb2.SharedFolderTeam] = []
...
def generate_shared_folder_records(sh_info: Tuple[bytes,  bytes],
                                   records: Iterable[Tuple[SyncDown_pb2.Record, bytes]]
                                   ) -> List[SyncDown_pb2.SharedFolderRecord]:
    result: List[SyncDown_pb2.SharedFolderRecord] = []
...
def generate_shared_folder(shared_folder: vault_types.SharedFolder,
                           has_key: bool) -> Tuple[SyncDown_pb2.SharedFolder, bytes]:
...
def get_test_records() -> Iterable[Union[
    vault_record.PasswordRecord, vault_record.TypedRecord, vault_record.FileRecord]]:
...
def get_test_shared_folders() -> Iterable[vault_types.SharedFolder]:
...
def get_test_teams() -> Iterable[vault_types.Team]:
    t = vault_types.Team()
...
def get_connected_auth() -> keeper_auth.KeeperAuth:
