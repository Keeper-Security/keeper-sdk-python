...
def get_vertex_content(vertex):
    ...
    try:
        ...
        logger.debug(f"Error getting vertex content: {e}")
        return_content = None
    return return_content
...
class TunnelDAG:
    def __init__(self, vault: vault_online.VaultOnline, encrypted_session_token, encrypted_transmission_key, record_uid: str, is_config=False):
        ...
        self.record = vault_record.PasswordRecord()
    ...
    def unlink_user_from_resource(self, user_uid, resource_uid) -> bool:
        ...
        if resource_vertex is None or not self.resource_belongs_to_config(resource_uid):
            logger.error(f"Resource {resource_uid} does not belong to the configuration")
            return False
    ...
    def print_tunneling_config(self, record_uid, pam_settings=None, config_uid=None):
        ...
        if content and content.get('allowedSettings'):
            allowed_settings = content['allowedSettings']
            logger.info(f"Settings configured for {record_uid}")
            port_forwarding = f"Enabled" if allowed_settings.get('portForwards') else \
                "Disabled"
            rotation = "Disabled" if (allowed_settings.get('rotation') and not allowed_settings['rotation']) else "Enabled"
            logger.info(f"\tRotation: {rotation}")
            logger.info(f"\tTunneling: {port_forwarding}")
            ...
            if config_id is not None:
                ...
                if config_content and config_content.get('allowedSettings'):
                    ...
                    config_port_forwarding = "Enabled" if (
                        config_allowed_settings.get('portForwards')) else \
                        "Disabled"
                    config_rotation = "Disabled" if (config_allowed_settings.get('rotation') and
                                                                       not config_allowed_settings['rotation']) else \
                        "Enabled"
                    logger.info(f"\tRotation: {config_rotation}")
                    logger.info(f"\tTunneling: {config_port_forwarding}")
