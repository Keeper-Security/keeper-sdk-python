...
import pydantic
from .dag_types import EdgeType, RefType
from . import dag_crypto
from .dag_edge import DAGEdge
from .exceptions import DAGVertexException, DAGDeletionException, DAGIllegalEdgeException, DAGKeyException
...
class DAGVertex:
    ...
    def __init__(self, dag: "DAG", uid: Optional[str] = None, name: Optional[str] = None,
        ...
        if uid is None:
            ...
            try:
                b = dag_crypto.urlsafe_str_to_bytes(uid)
                if len(b) != 16:
        ...
        if self.dag.get_root == self:
            ...
            for e in self.edges:
                if e.edge_type in [EdgeType.KEY, EdgeType.DATA]:
                    self._keychain.append(dag_crypto.generate_random_bytes(self.dag.__class__.UID_KEY_BYTES_SIZE))
                    break
    ...
    def add_data(self,
        ...
        # Make sure the vertex belongs before auto saving. If it does not belong, it is an orphan right now.
        # This only is checked if using this module to create the graph.
    ...
    def content_as_object(self,
                          meta_class: pydantic._internal._model_construction.ModelMetaclass) -> Optional[pydantic.BaseModel]:
        """
