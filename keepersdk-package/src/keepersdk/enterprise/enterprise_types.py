import abc
from typing import Tuple, Dict, Type, Iterator, Union
...
class EnterprisePlugin(abc.ABC):
    @abc.abstractmethod
    def put_entity(self, entity_type: int, data: bytes) -> None:
        pass

    @abc.abstractmethod
    def delete_entity(self, entity_type: int, data: bytes) -> None:
        pass
...
class EnterpriseStorage(EnterprisePlugin, abc.ABC):
    ...
    @abc.abstractmethod
    def get_all(self) -> Iterator[Tuple[int, bytes]]:
        pass
...
EnterpriseEntityMap: Dict[int, Type[Message]] = {
