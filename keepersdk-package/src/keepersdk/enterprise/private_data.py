import abc
import json
from typing import Generic, Dict, Optional, Tuple, Iterable, Any, List, Iterator

from . import enterprise_types
from .. import utils, crypto
from ..proto import enterprise_pb2
from ..storage.storage_types import T, K, KS, KO
...
class _IEnterpriseEntity(Generic[T, K], enterprise_types.IEnterpriseEntity[T, K], enterprise_types.IEnterprisePlugin[T], abc.ABC):
...
class _IEnterpriseLink(Generic[T, KS, KO], enterprise_types.IEnterpriseLink[T, KS, KO], enterprise_types.IEnterprisePlugin[T], abc.ABC):
    def __init__(self) -> None:
        super().__init__()
...
def _decrypt_encrypted_data(data: str, tree_key: bytes) -> Dict[str, Any]:
...
class NodeEntity(_IEnterpriseEntity[enterprise_types.Node, int]):
    ...
    def convert_entity(self, data, key) -> enterprise_types.Node:
        proto_entity = enterprise_pb2.Node()
        proto_entity.ParseFromString(data)
        name: str = ''
        if key and proto_entity.encryptedData:
            try:
                j = _decrypt_encrypted_data(proto_entity.encryptedData, key)
                name = j.get('displayname') or ''
            except Exception as e:
                utils.get_logger().debug('error decrypting EncryptedData: %s', e)

        entity = enterprise_types.Node(node_id=proto_entity.nodeId, name=name, parent_id=proto_entity.parentId,
        ...
        return enterprise_types.INode.clone(entity)
...
class RoleEntity(_IEnterpriseEntity[enterprise_types.Role, int]):
    ...
    def convert_entity(self, data, key) -> enterprise_types.Role:
        ...
        role = enterprise_types.Role(role_id=proto_entity.roleId, node_id=proto_entity.nodeId, name=name,  key_type=proto_entity.keyType,
                 visible_below=proto_entity.visibleBelow, new_user_inherit=proto_entity.newUserInherit,
                 role_type=proto_entity.roleType, encrypted_data=proto_entity.encryptedData)
        return enterprise_types.IRole.clone(role)
...
class UserEntity(_IEnterpriseEntity[enterprise_types.User, int]):
    ...
    def convert_entity(self, data, key) -> enterprise_types.User:
        proto_entity = enterprise_pb2.User()
        proto_entity.ParseFromString(data)
        full_name: str = ''
        if key and proto_entity.encryptedData:
            if proto_entity.keyType.lower() == 'no_key':
                full_name = proto_entity.encryptedData
            else:
                try:
                    j = _decrypt_encrypted_data(proto_entity.encryptedData, key)
                    full_name = j.get('displayname') or ''
                except Exception as e:
                    utils.get_logger().debug('error decrypting EncryptedData: %s', e)

        user = enterprise_types.User(enterprise_user_id=proto_entity.enterpriseUserId, username=proto_entity.username, node_id=proto_entity.nodeId,
                 full_name=full_name, job_title=proto_entity.jobTitle, status=proto_entity.status, lock=proto_entity.lock,
                 user_id=proto_entity.userId, account_share_expiration=proto_entity.accountShareExpiration, tfa_enabled=proto_entity.tfaEnabled,
                 transfer_acceptance_status=proto_entity.transferAcceptanceStatus, encrypted_data=proto_entity.encryptedData)
        return enterprise_types.IUser.clone(user)
...
class TeamEntity(_IEnterpriseEntity[enterprise_types.Team, str]):
    ...
    def convert_entity(self, data, key) -> enterprise_types.Team:
        proto_entity = enterprise_pb2.Team()
        proto_entity.ParseFromString(data)
        team_uid = utils.base64_url_encode(proto_entity.teamUid)
        e = enterprise_types.Team(team_uid=team_uid, name=proto_entity.name, node_id=proto_entity.nodeId, restrict_edit=proto_entity.restrictEdit,
        ...
        return enterprise_types.ITeam.clone(e)
...
class QueuedTeamEntity(_IEnterpriseEntity[enterprise_types.QueuedTeam, str]):
    ...
    def convert_entity(self, data, key) -> enterprise_types.QueuedTeam:
        proto_entity = enterprise_pb2.QueuedTeam()
        proto_entity.ParseFromString(data)
        team_uid = utils.base64_url_encode(proto_entity.teamUid)
        e = enterprise_types.QueuedTeam(team_uid=team_uid, name=proto_entity.name, node_id=proto_entity.nodeId, encrypted_data=proto_entity.encryptedData)
        return enterprise_types.IQueuedTeam.clone(e)
...
class TeamUserLink(_IEnterpriseLink[enterprise_types.TeamUser, str, int]):
    ...
    def convert_entity(self, data, key) -> enterprise_types.TeamUser:
        proto_entity = enterprise_pb2.TeamUser()
        proto_entity.ParseFromString(data)
        l = enterprise_types.TeamUser(team_uid=utils.base64_url_encode(proto_entity.teamUid),
                            enterprise_user_id=proto_entity.enterpriseUserId, user_type=proto_entity.userType)
        return enterprise_types.ITeamUser.clone(l)
...
class RoleUserLink(_IEnterpriseLink[enterprise_types.RoleUser, int, int]):
    ...
    def convert_entity(self, data, key) -> enterprise_types.RoleUser:
        proto_entity = enterprise_pb2.RoleUser()
        proto_entity.ParseFromString(data)
        l = enterprise_types.RoleUser(role_id=proto_entity.roleId, enterprise_user_id=proto_entity.enterpriseUserId)
        return enterprise_types.IRoleUser.clone(l)


class RolePrivilegeLink(enterprise_types.IEnterpriseLink[enterprise_types.RolePrivileges, int, int], enterprise_types.IEnterpriseDataPlugin):
    def __init__(self) -> None:
        super().__init__()
...
class RoleEnforcementLink(_IEnterpriseLink[enterprise_types.RoleEnforcement, int, str]):
    ...
    def convert_entity(self, data, key) -> enterprise_types.RoleEnforcement:
        proto_entity = enterprise_pb2.RoleEnforcement()
        proto_entity.ParseFromString(data)
        l = enterprise_types.RoleEnforcement(role_id=proto_entity.roleId, enforcement_type=proto_entity.enforcementType,
                                             value=proto_entity.value)
        return enterprise_types.IRoleEnforcement.clone(l)
...
class RoleTeamLink(_IEnterpriseLink[enterprise_types.RoleTeam, int, str]):
    ...
    def convert_entity(self, data, key) -> enterprise_types.RoleTeam:
        proto_entity = enterprise_pb2.RoleTeam()
        proto_entity.ParseFromString(data)
        l = enterprise_types.RoleTeam(role_id=proto_entity.role_id, team_uid=utils.base64_url_encode(proto_entity.teamUid))
        return enterprise_types.RoleTeam.clone(l)
...
class LicenseEntity(_IEnterpriseEntity[enterprise_types.License, int]):
    ...
    def convert_entity(self, data, key) -> enterprise_types.License:
...
class ManagedNodeLink(_IEnterpriseLink[enterprise_types.ManagedNode, int, int]):
    ...
    def convert_entity(self, data, key) -> enterprise_types.ManagedNode:
        proto_entity = enterprise_pb2.ManagedNode()
        proto_entity.ParseFromString(data)
        l = enterprise_types.ManagedNode(role_id=proto_entity.roleId, managed_node_id=proto_entity.managedNodeId,
                                         cascade_node_management=proto_entity.cascadeNodeManagement)
        return enterprise_types.IManagedNode.clone(l)
...
class ManagedCompanyEntity(_IEnterpriseEntity[enterprise_types.ManagedCompany, int]):
    ...
    def convert_entity(self, data, key) -> enterprise_types.ManagedCompany:
...
class QueuedTeamUserLink(enterprise_types.IEnterpriseLink[enterprise_types.QueuedTeamUser, str, int], enterprise_types.IEnterpriseDataPlugin):
    def __init__(self) -> None:
        super().__init__()
...
class UserAliasLink(_IEnterpriseLink[enterprise_types.UserAlias, int, str]):
    ...
    def convert_entity(self, data, key):
