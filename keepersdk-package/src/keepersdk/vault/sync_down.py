...
from .storage_types import (
    ...
    SharedFolderUserType, BreachWatchRecord, UserSettings, IPendingShares)
...
def sync_down_request(auth: keeper_auth.KeeperAuth,
    ...
    while not done:
        if user_settings.continuation_token:
            rq.continuationToken = user_settings.continuation_token
        ...
        if len(response.removedSharedFolders) > 0:
            ...
            # delete sub folders
            sf_to_remove = set(removed_shared_folders)
            sffs = [x.folder_uid for x in storage.folders.get_all()
                    if x.shared_folder_uid and x.shared_folder_uid in sf_to_remove]
            storage.folders.delete_uids(sffs)
            storage.folder_records.delete_links_for_subjects(sffs)
            del rec_links
        ...
        if len(response.recordMetaData) > 0:
            record_owners.update(((utils.base64_url_encode(x.recordUid), RecordOwner(x.owner, x.ownerAccountUid))
                                  for x in response.recordMetaData))
        ...
        if len(response.records) > 0:
            def to_record(rec: SyncDown_pb2.Record) -> StorageRecord:
                ...
                if record.record_uid in record_owners:
                    ro = record_owners.pop(record.record_uid)
                    record.owner = ro.owner
                    record.owner_account_id = ro.owner_account_id
                return record
        ...
        if len(response.sharedFolderRecords) > 0:
            record_owners.update(((utils.base64_url_encode(x.recordUid), RecordOwner(x.owner, x.ownerAccountUid))
                                  for x in response.sharedFolderRecords))
        ...
        if len(response.removedSharedFolderTeams) > 0:
            result.add_shared_folders((utils.base64_url_encode(x.sharedFolderUid) for x in response.removedSharedFolderTeams))
    ...
    if len(record_owners) > 0:
        records = []
        for record_uid, record_owner in record_owners.items():
            e_record = storage.records.get_entity(record_uid)
            if e_record:
                if e_record.owner_account_id != record_owner.owner_account_id or e_record.owner != record_owner.owner:
                    e_record.owner = record_owner.owner
                    e_record.owner_account_id = record_owner.owner_account_id
