...
def decryptKeeperKey(auth_context: keeper_auth.AuthContext, encrypted: bytes, key_type: record_pb2.RecordKeyType) -> bytes:
...
def sync_down_request(auth: keeper_auth.KeeperAuth,
    ...
    while not done:
        ...
        if len(response.removedSharedFolders) > 0:
            ...
            rec_links: List[storage_types.IUidLink] = []
            ...
            task.add_shared_folders(removed_shared_folders)
            task.add_records((x.subject_uid() for x in rec_links))
        ...
        if len(response.recordMetaData) > 0:
            def to_record_key(rmd: SyncDown_pb2.RecordMetaData) -> Optional[StorageRecordKey]:
                ...
                try:
                    record_key = decryptKeeperKey(auth.auth_context, record_key, key_type)
        ...
        if len(response.teams) > 0:
            ...
            def to_team(sync_down_team: SyncDown_pb2.Team) -> Optional[StorageTeam]:
                try:
                    team_key = decryptKeeperKey(auth.auth_context, sync_down_team.teamKey, sync_down_team.teamKeyType)
        ...
        if len(response.sharedFolders) > 0:
            ...
            def to_shared_folder_key(shared_folder: SyncDown_pb2.SharedFolder) -> Optional[StorageSharedFolderKey]:
                if len(shared_folder.sharedFolderKey) > 0:
                    try:
                        shared_folder_key = decryptKeeperKey(auth.auth_context, shared_folder.sharedFolderKey, shared_folder.keyType)
        ...
        if len(response.removedSharedFolderTeams) > 0:
            task.add_shared_folders((utils.base64_url_encode(x.sharedFolderUid) for x in response.removedSharedFolderTeams))
            storage.shared_folder_keys.delete_links((storage_types.UidLink(
                utils.base64_url_encode(x.sharedFolderUid),
