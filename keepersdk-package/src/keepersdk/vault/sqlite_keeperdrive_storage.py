...
def keeper_drive_table_schemas(owner_column: str, owner_type: Type[sqlite_dao.KeyTypes]
    ...
    record_data_schema = sqlite_dao.TableSchema.load_schema(
        kd.KDRecordData, 'record_uid', owner_column=owner_column, owner_type=owner_type)
    ...
    record_summary_schema = sqlite_dao.TableSchema.load_schema(
        kd.KDRecordSummary, 'record_uid', owner_column=owner_column, owner_type=owner_type)
    ...
    return (settings_schema, folder_schema, folder_key_schema, folder_access_schema,
            record_data_schema, non_shared_schema, record_access_schema,
            record_link_schema, bw_record_schema, security_score_schema, bw_sec_schema,
            folder_record_schema, record_summary_schema,
            list_chunk_schema)
...
class SqliteKeeperDriveStorage(IKeeperDriveStorage):
    def __init__(self, get_connection: Callable[[], sqlite3.Connection], vault_owner: bytes, *,
        ...
        (settings_schema, folder_schema, folder_key_schema, folder_access_schema,
         record_data_schema, non_shared_schema, record_access_schema,
         record_link_schema, bw_record_schema, security_score_schema, bw_sec_schema,
         folder_record_schema, record_summary_schema,
         list_chunk_schema) = schemas
        ...
        self._record_data = sqlite.SqliteEntityStorage(
            self.get_connection, record_data_schema, owner=self.vault_owner)
        ...
        self._record_summaries = sqlite.SqliteEntityStorage(
            self.get_connection, record_summary_schema, owner=self.vault_owner)
    ...
    def clear_all(self) -> None:
        ...
        self._record_summaries.delete_all()
