...
) -> Tuple[List[str], List[record_endpoints_pb2.RecordAdd]]:
...
def create_nsf_records_batch(
        ...
        *,
        ...
        folder_key_cache: Optional[Dict[str, bytes]] = None) -> List[NsfModifyResult]:
    ...
    record_uids, record_adds = _prepare_nsf_record_add_messages(
        vault, specs, folder_key_cache=folder_key_cache)
    response = _execute_nsf_records_add(vault, record_adds)
...
def create_nsf_records(
    ...
    for batch_start in range(0, len(specs), NSF_RECORD_ADD_BATCH_LIMIT):
        ...
        is_last = batch_start + len(batch) >= len(specs)
        all_results.extend(create_nsf_records_batch(
            vault,
            batch,
            request_sync=request_sync and is_last,
            folder_key_cache=folder_key_cache,
        ))
    return all_results
...
def get_nsf_access_details_from_cache(vault: VaultOnline, record_uid: str = None, folder_uid: str = None) -> Dict[str, Any]:
    view = _nsf_view(vault)
    if record_uid:
        record = view.get_record(record_uid)
        if not record:
            raise NsfError(f'NSF record not found: {record_uid}')
        return vault.nsf_data._record_access_details.get(record_uid) or {}
    if folder_uid:
        folder = view.get_folder(folder_uid)
        if not folder:
            raise NsfError(f'NSF folder not found: {folder_uid}')
        return vault.nsf_data._folder_access_details.get(folder_uid) or {}
    raise ValueError("At least one of record_uid or folder_uid is required")
