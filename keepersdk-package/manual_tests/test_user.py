...
class MyTestCase(unittest.TestCase):
    ...
    def test_folder_create(self):
        ...
        self.assertEqual(vault.vault_data.record_count, 0)
        self.assertEqual(vault.vault_data.shared_folder_count, 0)
        ...
        sf1 = vault.vault_data.get_shared_folder(shared_folder_uid)
        self.assertIsNotNone(sf1)

        f1 = vault.vault_data.get_folder(shared_folder_uid)
        self.assertIsNotNone(f1)
        self.assertEqual(f1.folder_type, 'shared_folder')

        f2 = vault.vault_data.get_folder(sub_folder_uid)
        self.assertIsNotNone(f2)
        self.assertEqual(f2.folder_type, 'shared_folder_folder')

        f3 = vault.vault_data.get_folder(user_folder_uid)
    ...
    def test_record_update(self):
        ...
        typed_record_uid = next((x.record_uid for x in vault.vault_data.records() if x.version == 3 and x.record_type == 'login'), None)
        ...
        legacy_record_uid = next((x.record_uid for x in vault.vault_data.records() if x.version == 2), None)
        ...
        typed_record = vault.vault_data.load_record(typed_record_uid)
        self.assertIsInstance(typed_record, vault_record.TypedRecord)
        self.assertEqual(typed_record.record_type, 'login')
        ...
        legacy_record = vault.vault_data.load_record(legacy_record_uid)
